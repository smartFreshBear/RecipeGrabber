...
class TestTrainingInterfaceLogic(unittest.TestCase):
    def test_get_values_data(self):
        values = get_values(cells_range='A:C', target_spreadsheet=TEST_INSERT_SPREADSHEET_ID)
    ...
    def test_insert_values(self):
        ...
        insert_response = insert_values(cells, TEST_INSERT_SPREADSHEET_ID)

        assert insert_response is not None, 'Request returned bad response'

        cells_range = insert_response['updates']['updatedRange']
        values = get_values(cells_range=cells_range, target_spreadsheet=TEST_INSERT_SPREADSHEET_ID)

        assert values == cells

        delete_response = delete_values(cells_range, TEST_INSERT_SPREADSHEET_ID)
